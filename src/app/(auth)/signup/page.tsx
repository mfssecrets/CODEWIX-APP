...
import { ArrowRight, Loader2, Mail, User, ArrowLeft, KeyRound, Check } from 'lucide-react';
  ...
  const handleVerify = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/auth/otp-signup', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: email.trim(), name: name.trim(), otp: code }),
      });
      const data = await res.json();
  ...
  return (
    <AuthCard subtitle="Create your account">
        ...
        {[1, 2, 3].map((s) => (
          <div key={s} className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-bold transition-all duration-300 ${
              step > s ? 'bg-emerald-500 text-white' : step === s ? 'bg-violet-600 text-white' : 'bg-slate-100 text-slate-400'
            }`}>
              {step > s ? <Check className="w-3.5 h-3.5" strokeWidth={3} /> : s}
            </div>
            {s < 3 && <div className={`w-8 h-0.5 rounded-full transition-all duration-300 ${step > s ? 'bg-emerald-400' : 'bg-slate-200'}`} />}
          </div>
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
        {step === 1 && (
          <motion.div key="s1" initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 12 }} transition={{ duration: 0.3 }} className="flex flex-col gap-3.5">
            <div className="flex flex-col gap-1.5">
              <label className="text-[12.5px] font-medium text-slate-600">Name</label>
        ...
        {step === 2 && (
          <motion.div key="s2" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: 0.3 }} className="flex flex-col gap-4">
            <button onClick={() => { setStep(1); setOtp(Array(6).fill('')); setError(''); }} className="flex items-center gap-1.5 text-[13px] text-slate-500 hover:text-slate-700">
            ...
            <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} onClick={handleVerify} disabled={loading || otp.some((d) => !d)}
              className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 text-white text-[13.5px] font-semibold shadow-lg shadow-violet-500/25 hover:shadow-xl hover:shadow-violet-500/35 disabled:opacity-60 transition-all">
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <><KeyRound className="w-4 h-4" strokeWidth={1.8} /> Verify & Create Account</>}
            </motion.button>
          </motion.div>
        )}
        {step === 3 && (
          <motion.div key="s3" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center py-6">
            <div className="w-14 h-14 rounded-full bg-emerald-50 flex items-center justify-center mb-4">
              <Check className="w-7 h-7 text-emerald-500" strokeWidth={1.8} />
            </div>
            <p className="text-[15px] font-semibold text-slate-800">Account created!</p>
            <p className="text-[13px] text-slate-500">Redirecting...</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {step < 3 && (
        <p className="text-center text-[13px] text-slate-500 mt-6">
          ...
          <Link href="/signin" className="font-semibold text-violet-600 hover:text-violet-700 transition-colors">Sign in</Link>
        </p>
      )}
    </AuthCard>
  );
