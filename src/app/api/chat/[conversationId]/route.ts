...
export async function GET(req: NextRequest, { params }: { params: Promise<{ conversationId: string }> }) {
  ...
  const convo = await db.conversation.findFirst({ where: { id: conversationId, userId }, include: { messages: { orderBy: { timestamp: 'asc' }, include: { attachments: true } } } });
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ conversationId: string }> }) {
  ...
  if (body.title !== undefined) {
    await db.conversation.updateMany({ where: { id: conversationId, userId }, data: { title: body.title } });
  }
  return NextResponse.json({ success: true });
}